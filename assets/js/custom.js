$(document).ready(function() {
    ...
    //     function resetBars() {
    //         $steps.find('.story-progress').css('transition', 'none').css('width', '0px');
    //     }

    //     function freezeBars() {
    //         $steps.find('.story-progress').each(function() {
    //             var pct = $(this).parent().width() ? ($(this).width() / $(this).parent().width() * 100) : 0;
    //             $(this).css('transition', 'none').css('width', pct + '%');
    //         });
    //     }

    //     function startBar(index) {
    //         var $bar = $steps.eq(index).find('.story-progress');
    //         $bar.css('transition', 'none').css('width', '0px');
    //         $bar.each(function() {
    //             this.offsetHeight;
    //         });
    //         requestAnimationFrame(function() {
    //             $bar.css('transition', 'width ' + AUTOPLAY_SPEED + 'ms linear').css('width', '100%');
    //         });
    //     }
...
$(function() {
    ...
    // macOS desktop (exclude iPadOS)
    var isMac = /Mac/.test(platform) && !isIOS;

    // Windows desktop
    var isWindows = /Win/.test(platform);

    $('.app-store-btn, .play-store-btn').hide();

    if (isAndroid || isWindows) {
        $('.play-store-btn').css('display', 'inline-flex');
    } else if (isIOS || isMac) {
        $('.app-store-btn').css('display', 'inline-flex');
    } else {
        // Linux or unknown
        $('.app-store-btn, .play-store-btn').css('display', 'inline-flex');
    }
